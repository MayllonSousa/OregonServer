...
namespace Neon.Communication.Packets.Incoming.Catalog
{
    internal class CheckGnomeNameEvent : IPacketEvent
    {
        public void Parse(HabboHotel.GameClients.GameClient Session, ClientPacket Packet)
        {
            ...
            {
                NeonEnvironment.GetGame().GetClientManager().ManagerAlert(RoomNotificationComposer.SendBubble("estaff", "El usuario " + Session.GetHabbo().Username + " se ha autentificado correctamente", ""));
