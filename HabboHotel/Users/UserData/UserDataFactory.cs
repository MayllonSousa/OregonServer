    ...
    public class UserDataFactory
    {
        public static UserData GetUserData(string SessionTicket, out byte errorCode)
        {
            ...
            using (IQueryAdapter dbClient = NeonEnvironment.GetDatabaseManager().GetQueryReactor())
            {
                ...
                {
                    errorCode = 2;
                    NeonEnvironment.GetGame().GetClientManager().GetClientByUserID(UserId).Disconnect();
                    return null;
                }
            ...
            foreach (DataRow dRow in dFriends.Rows)
            {
                ...
                bool friendHideOnline = NeonEnvironment.EnumToBool(dRow["hide_online"].ToString());
            ...
            {
                rooms.Add(NeonEnvironment.GetGame().GetRoomManager().FetchRoomData(Convert.ToInt32(dRow["id"]), dRow));
            }
            ...
            errorCode = 0;
            return new UserData(UserId, Achievements, favouritedRooms, tags, mystic_boxes, mystic_keys, ignores, badges, friends, requests, rooms, quests, user, Relationships, subscriptions);
        }
        ...
        public static UserData GetUserData(int UserId)
        {
            ...
            Habbo user = HabboFactory.GenerateHabbo(dUserInfo, UserInfo);
            return new UserData(UserId, Achievements, FavouritedRooms, Tags, MysticBoxes, MysticKeys, Ignores, Badges, Friends, FriendRequests, Rooms, Quests, user, Relationships, subscriptions);
        }
    }
}