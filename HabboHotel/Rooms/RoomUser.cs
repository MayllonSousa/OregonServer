        ...
        public void OnChat(int Colour, string Message, bool Shout)
        {
            ...
            for (int i = 0; i < mensaje.Length; i++)
            {
                if (mensaje[i].Contains("@"))
                {
                    ...
                    {
                        TargetClient.SendMessage(RoomNotificationComposer.SendBubble("mentiond", GetClient().GetHabbo().Username + " te ha mencionado en una sala: " + Message + ".", "navigator/goto/" + GetClient().GetHabbo().CurrentRoom + ""));
                        GetClient().SendWhisper("Haz mencionado a un usuario");
                    }
                }
            }
