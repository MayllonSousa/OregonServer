...
namespace Neon.HabboHotel.Rooms.AI.Types
{
    public class PetBot : BotAI
    {
        ...
        public override void OnUserEnterRoom(RoomUser User)
        {
            if (User.GetClient() != null && User.GetClient().GetHabbo() != null)
            {
                ...
                if (Pet != null)
                {
                    ...
                    {
                        string[] Speech = NeonEnvironment.GetGame().GetChatManager().GetPetLocale().GetValue("welcome.speech.pet" + Pet.PetData.Type);
        ...
        public override void OnUserSay(RoomUser User, string Message)
        {
            ...
            if ((Message.ToLower().StartsWith(Pet.PetData.Name.ToLower() + " ") && User.GetClient().GetHabbo().Username.ToLower() == Pet.PetData.OwnerName.ToLower()) || (Message.ToLower().StartsWith(Pet.PetData.Name.ToLower() + " ") && NeonEnvironment.GetGame().GetChatManager().GetPetCommands().TryInvoke(Message.Substring(Pet.PetData.Name.ToLower().Length + 1).ToLower()) == 8))
            {
                string Command = Message.Substring(Pet.PetData.Name.ToLower().Length + 1);

                int r = RandomNumber.GenerateRandom(1, 8); // Made Random
                if (Pet.PetData.Energy > 10 && r < 6 || Pet.PetData.Level > 15 || NeonEnvironment.GetGame().GetChatManager().GetPetCommands().TryInvoke(Command) == 8)
                {
                    RemovePetStatus(); // Remove Status

                    switch (NeonEnvironment.GetGame().GetChatManager().GetPetCommands().TryInvoke(Command))
                    {
                        #region free
                        ...
                        default:
                            string[] Speech = NeonEnvironment.GetGame().GetChatManager().GetPetLocale().GetValue("pet.unknowncommand");
