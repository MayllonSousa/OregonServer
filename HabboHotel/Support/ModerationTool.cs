...
//                {
//                    dbClient.SetQuery("INSERT INTO moderation_tickets (score,type,status,sender_id,reported_id,moderator_id,message,room_id,room_name,timestamp) VALUES (1,'" + Category + "','open','" + Session.GetHabbo().Id + "','" + ReportedUser + "','0',@message,'0','','" + NeonEnvironment.GetUnixTimestamp() + "')");
...
//                Ticket = new SupportTicket(TicketId, 1, 7, Session.GetHabbo().Id, ReportedUser, Message, 0, "", NeonEnvironment.GetUnixTimestamp(), Messages);
...
//            RoomData Data = NeonEnvironment.GetGame().GetRoomManager().GenerateRoomData(Session.GetHabbo().CurrentRoomId);

//            using (IQueryAdapter dbClient = NeonEnvironment.GetDatabaseManager().GetQueryReactor())
//            {
//                dbClient.SetQuery("INSERT INTO moderation_tickets (score,type,status,sender_id,reported_id,moderator_id,message,room_id,room_name,timestamp) VALUES (1,'" + Category + "','open','" + Session.GetHabbo().Id + "','" + ReportedUser + "','0',@message,'" + Data.Id + "',@name,'" + NeonEnvironment.GetUnixTimestamp() + "')");
//                dbClient.AddParameter("message", Message);
...
//            Ticket = new SupportTicket(TicketId, 1, 7, Session.GetHabbo().Id, ReportedUser, Message, Data.Id, Data.Name, NeonEnvironment.GetUnixTimestamp(), Messages);
//            Tickets.Add(Ticket);
//            SendTicketToModerators(Ticket);
//        }
...
//            GameClient Client = NeonEnvironment.GetGame().GetClientManager().GetClientByUserID(Ticket.SenderId);
...
//        {
//            NeonEnvironment.GetGame().GetClientManager().SendMessage(new ModeratorSupportTicketComposer(Ticket), "mod_tool");
//            NeonEnvironment.GetGame().GetClientManager().StaffAlert(RoomNotificationComposer.SendBubble("tickets", "Hay una nueva petición de ayuda, revísala cuanto antes.", ""));
